...
export default function Billing({
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(300px,0.96fr)]"><div className="space-y-6"><div className="grid gap-4 sm:grid-cols-3"><Metric label="Total Billed" value={currency(totalBilled)} /><Metric label="Total Paid" value={currency(totalPaid)} emphasis /><Metric label="Balance Due" value={currency(balanceDue)} /></div><Panel><SectionHeading title="Proposals" subtitle="Event proposals and quotes" action={<button onClick={onGenerateProposal} className="inline-flex items-center gap-2 rounded-md bg-[#cc2622] px-4 py-2 text-sm font-semibold text-white hover:bg-[#a01f1a]"><Plus className="h-4 w-4" />Generate Proposal</button>} />{eventQuotations.length ? <DataTable headers={['Proposal #', 'Created', 'Status', 'Amount', 'Actions']} rows={eventQuotations.map((quotation) => [quotation.quotationNumber, formatDate(quotation.createdAt), <StatusBadge key={`${quotation.quotationId}-status`} className={proposalStatusStyles[quotation.quotationStatus]} label={quotation.quotationStatus} />, currency(quotation.total), <button key={`${quotation.quotationId}-view`} aria-label={`View proposal ${quotation.quotationNumber}`} className="text-slate-600 hover:text-[#cc2622]"><Eye className="ml-auto h-4 w-4" /></button>])} /> : <EmptyBox icon={<FileText className="h-9 w-9" />} message="No Proposals Created Yet" />}</Panel><Panel><SectionHeading title="Billing Details" subtitle="Items included in the current event total" />{currentQuotation?.lineItems.length ? <DataTable headers={['Item', 'Type', 'Quantity', 'Unit Price', 'Total']} rows={currentQuotation.lineItems.map((item) => [item.lineItemDescription, lineItemLabels[item.lineItemType], Number(item.quantity ?? 0).toLocaleString(), currency(item.unitPriceAtQuotation), currency(item.totalPrice)])} /> : <EmptyBox icon={<ReceiptText className="h-9 w-9" />} message="No Billed Items Yet" />}</Panel><Panel><SectionHeading title="Invoices" subtitle="Invoice history for this event" action={<button onClick={onGenerateInvoice} className="inline-flex items-center gap-2 rounded-md bg-[#cc2622] px-4 py-2 text-sm font-semibold text-white hover:bg-[#a01f1a]"><Plus className="h-4 w-4" />Generate Invoice</button>} />{eventInvoices.length ? <DataTable headers={['Invoice #', 'Created', 'Status', 'Amount Due', 'Paid', 'Balance']} rows={eventInvoices.map((invoice) => [invoice.invoiceNumber, formatDate(invoice.createdAt), <StatusBadge key={`${invoice.invoiceId}-status`} className={invoiceStatusStyles[invoice.invoiceStatus]} label={invoice.invoiceStatus} />, currency(invoice.amountDue), currency(invoice.amountPaid), currency(invoice.balance)])} /> : <EmptyBox icon={<ReceiptText className="h-9 w-9" />} message="No Invoices Created Yet" />}</Panel><Panel><SectionHeading title="Payment History" subtitle="Payments received for this event" action={<OutlineButton><Banknote className="h-4 w-4" />Record Payment</OutlineButton>} />{paidInvoices.length ? <DataTable headers={['Invoice #', 'Paid', 'Balance', 'Status']} rows={paidInvoices.map((invoice) => [invoice.invoiceNumber, currency(invoice.amountPaid), currency(invoice.balance), <StatusBadge key={`${invoice.invoiceId}-payment-status`} className={invoiceStatusStyles[invoice.invoiceStatus]} label={invoice.invoiceStatus} />])} /> : <EmptyBox icon={<Banknote className="h-9 w-9" />} message="No Payments Recorded Yet" />}</Panel></div><EventSidebarActions eventId={event.id} onGenerateInvoice={onGenerateInvoice} onGenerateProposal={onGenerateProposal} /></div>
}
